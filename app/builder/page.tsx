...
export default function BuilderPage() {
  ...
  const slugify = (s: string) =>
    s.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '').slice(0, 24) || 'var'
  ...
  const updateComponentField = (id: string, field: keyof CustomComponentConfig, value: any) => {
    const updatedComps = calculator.components.map((c) => {
      ...
      if (field === 'label' && typeof value === 'string') {
        const autoNames = ['value', 'value_1', 'x', 'y', 'radius', 'height', 'weight', 'width', 'price', 'pct', 'people', 'principal', 'rate', 'years', 'bill']
        if (autoNames.includes(c.name) || c.name.startsWith('var_') || c.name.endsWith('_copy')) {
