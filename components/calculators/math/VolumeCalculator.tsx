...
export default function VolumeCalculator() {
  ...
  const calculate = () => {
    ...
    let vol = 0
    switch (shape) {
      case 'cube': { const s = g('side'); if (s <= 0) { setError('Side must be positive'); return }; vol = s ** 3; break }
      case 'sphere': { const r = g('radius'); if (r <= 0) { setError('Radius must be positive'); return }; vol = (4 / 3) * Math.PI * r ** 3; break }
      case 'cylinder': { const r = g('radius'), h = g('height'); if (r <= 0 || h <= 0) { setError('Dimensions must be positive'); return }; vol = Math.PI * r ** 2 * h; break }
      case 'cone': { const r = g('radius'), h = g('height'); if (r <= 0 || h <= 0) { setError('Dimensions must be positive'); return }; vol = (1 / 3) * Math.PI * r ** 2 * h; break }
      case 'pyramid': { const l = g('length'), w = g('width'), h = g('height'); if (l <= 0 || w <= 0 || h <= 0) { setError('Dimensions must be positive'); return }; vol = (1 / 3) * l * w * h; break }
      case 'prism': { const l = g('length'), w = g('width'), h = g('height'); if (l <= 0 || w <= 0 || h <= 0) { setError('Dimensions must be positive'); return }; vol = l * w * h; break }
    }
    setResult(parseFloat(vol.toFixed(6)).toString())
  }
