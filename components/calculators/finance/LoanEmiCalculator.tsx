  ...
  return (
    <FormCalculatorShell title="Loan EMI Calculator" badge="FINANCE">
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        <ResultDisplay label="Monthly EMI" value={`$${Math.round(emi).toLocaleString()}`} large />
        <ResultDisplay label="Total Interest" value={`$${Math.round(totalInterest).toLocaleString()}`} large />
      </div>

      <div className="mt-3">
        <ResultDisplay label="Total Payment" value={`$${Math.round(totalPayment).toLocaleString()}`} />
      </div>
