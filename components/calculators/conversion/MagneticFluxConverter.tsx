...
export default function MagneticFluxConverter() {
  ...
  return (
    <FormCalculatorShell title="Magnetic Flux Converter" subtitle="Weber · maxwell · tesla·m²" badge="CONVERSION">
      <RetroInput label="Value" value={value} onChange={setValue} placeholder="1" id="mag-val" />
      <div className="grid grid-cols-2 gap-3">
        <RetroSelect label="From" value={from} onChange={setFrom} options={options} id="mag-from" />
        <RetroSelect label="To" value={to} onChange={setTo} options={options} id="mag-to" />
      </div>
      {valid && (
        <>
          <div className="mt-4">
