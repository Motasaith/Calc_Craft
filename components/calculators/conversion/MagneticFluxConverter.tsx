...
export default function MagneticFluxConverter() {
  ...
  const [fromUnit, setFromUnit] = useState('Wb')
  const [toUnit, setToUnit] = useState('Mx')
              ...
              {chartData.length > 0 && (
                <div className="rounded-xl border border-neutral-350 bg-[#cbd8ca]/30 p-4">
                  <p className="mb-3 text-[9px] font-bold uppercase tracking-wider text-neutral-600 font-mono">
                    Relative Scale (Compared to 1 Wb)
                  </p>
                    ...
                    {chartData.map((bar, idx) => (
                      <div key={idx} className="space-y-1">
                        <div className="flex justify-between text-[10px] font-mono text-neutral-600">
                          ...
                          <span>{bar.baseConvVal.toLocaleString(undefined, { maximumFractionDigits: 4 })} Wb</span>
                        </div>
