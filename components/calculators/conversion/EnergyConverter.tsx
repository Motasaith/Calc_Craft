...
export default function EnergyConverter() {
  ...
  const singleResult = useMemo(() => {
    if (!isValid) return 0
    return convertUnits(value, fromUnit, toUnit, 'energy')
  ...
  const allConversions = useMemo(() => {
    if (!isValid) return []
    return units.map(u => {
      const converted = convertUnits(value, fromUnit, u, 'energy')
      return {
        unit: u,
  ...
  const chartData = useMemo(() => {
    ...
    return referenceUnits.map((u) => {
      const baseConvVal = convertUnits(baseVal, u, units[0], 'energy')
      return {
        unit: u,
        label: unitLabels[u] || u,
        baseConvVal,
  ...
  return (
    <FormCalculatorShell title="Energy Converter" subtitle="Convert between different units of energy" badge="CONVERSION">
