...
export default function RadiationConverter() {
  ...
  return (
    <FormCalculatorShell title="Radiation Converter" subtitle="Gray · rad · sievert · rem" badge="CONVERSION">
      <RetroInput label="Value" value={value} onChange={setValue} placeholder="1" id="rad-val" />
      <div className="grid grid-cols-2 gap-3">
        <RetroSelect label="From" value={from} onChange={setFrom} options={options} id="rad-from" />
        <RetroSelect label="To" value={to} onChange={setTo} options={options} id="rad-to" />
      </div>
      {valid && (
        <>
          <div className="mt-4">
