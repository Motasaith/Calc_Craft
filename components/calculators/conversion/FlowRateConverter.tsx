...
export default function FlowRateConverter() {
  ...
  return (
    <FormCalculatorShell title="Flow Rate Converter" subtitle="L/s · GPM · m³/h · CFM" badge="CONVERSION">
      <RetroInput label="Value" value={value} onChange={setValue} placeholder="1" id="flow-val" />
      <div className="grid grid-cols-2 gap-3">
        <RetroSelect label="From" value={from} onChange={setFrom} options={options} id="flow-from" />
        <RetroSelect label="To" value={to} onChange={setTo} options={options} id="flow-to" />
      </div>
      {valid && (
        <>
          <div className="mt-4">
