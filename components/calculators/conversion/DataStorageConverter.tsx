...
export default function DataStorageConverter() {
  ...
  const singleResult = useMemo(() => {
    if (!isValid) return 0
    return convertUnits(value, fromUnit, toUnit, 'data')
  ...
  const allConversions = useMemo(() => {
    if (!isValid) return []
    return units.map(u => {
      const converted = convertUnits(value, fromUnit, u, 'data')
      return {
        unit: u,
  ...
  const chartData = useMemo(() => {
    ...
    return referenceUnits.map((u) => {
      const baseConvVal = convertUnits(baseVal, u, units[0], 'data')
      return {
        unit: u,
        label: unitLabels[u] || u,
        baseConvVal,
  ...
  return (
    <FormCalculatorShell title="Data Storage Converter" subtitle="Convert between different units of data storage" badge="CONVERSION">
