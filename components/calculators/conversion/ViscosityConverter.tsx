...
const UNITS: Record<string, number> = {
  'Pa·s': 1, 'cP': 0.001, 'P': 0.1, 'lb/ft·s': 1.48816,
}
...
export default function ViscosityConverter() {
  ...
  return (
    <FormCalculatorShell title="Viscosity Converter" subtitle="Pa·s · cP · poise · lb/ft·s" badge="CONVERSION">
      <RetroInput label="Value" value={value} onChange={setValue} placeholder="1" id="visc-val" />
      <div className="grid grid-cols-2 gap-3">
        <RetroSelect label="From" value={from} onChange={setFrom} options={options} id="visc-from" />
        <RetroSelect label="To" value={to} onChange={setTo} options={options} id="visc-to" />
      </div>
      {valid && (
        <>
          <div className="mt-4">
