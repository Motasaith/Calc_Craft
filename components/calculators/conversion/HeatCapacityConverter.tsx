...
const UNITS: Record<string, number> = {
  'J/K': 1, 'cal/°C': 4.184, 'BTU/°F': 1899.1,
}

export default function HeatCapacityConverter() {
  const [value, setValue] = useState('1')
  const [from, setFrom] = useState('J/K')
  const [to, setTo] = useState('cal/°C')
  ...
  return (
    <FormCalculatorShell title="Heat Capacity Converter" subtitle="J/K · cal/°C · BTU/°F" badge="CONVERSION">
      <RetroInput label="Value" value={value} onChange={setValue} placeholder="1" id="heat-val" />
      <div className="grid grid-cols-2 gap-3">
        <RetroSelect label="From" value={from} onChange={setFrom} options={options} id="heat-from" />
        <RetroSelect label="To" value={to} onChange={setTo} options={options} id="heat-to" />
      </div>
      {valid && (
        <>
          <div className="mt-4">
